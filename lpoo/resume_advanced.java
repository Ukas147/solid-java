/**
 * Interfaces, Herança, Composição, Agregação, Associação, Exceções, Singleton e Outros Padrões em Java
 *
 * Este código implementa um sistema de **Parque Selvagem**, aplicando conceitos avançados da POO.
 *
 * 🔹 **Interface (`Animal`)**:
 *    - Define um contrato para todos os animais do parque.
 *
 * 🔹 **Herança e Abstração (`AnimalSelvagem`)**:
 *    - `AnimalSelvagem` é uma classe abstrata que implementa `Animal` e define comportamentos adicionais como `respirar()` e `habitat()`.
 *    - `Leao` é uma subclasse concreta que sobrescreve `fazerSom()`, `mover()` e `habitat()`.
 *
 * 🔹 **Composição (`Setor`)**:
 *    - `Setor` contém uma lista de `Animal`, ou seja, **não pode existir sem o parque**.
 *
 * 🔹 **Agregação (`Funcionario`)**:
 *    - `Funcionario` pertence ao parque, mas **pode existir sem ele**.
 *
 * 🔹 **Associação (`Veterinario`)**:
 *    - `Veterinario` pode examinar qualquer `Animal`, mas não os possui.
 *
 * 🔹 **Exceções Personalizadas (`SetorNaoEncontradoException`)**:
 *    - Exceção customizada para indicar quando um setor não é encontrado.
 *
 * 🔹 **Singleton (`ParqueSelvagem`)**:
 *    - Garante que **apenas um parque exista** na aplicação.
 *
 * 🔹 **Try-Catch-Finally (`ParqueSelvagem.buscarSetor()`)**:
 *    - Tratamento de erro ao buscar setores inexistentes.
 *
 * 🔹 **Inner Class (`ParqueSelvagem.Estatisticas`)**:
 *    - Classe interna para calcular estatísticas do parque.
 *
 * 🔹 **Overloading (`Leao.fazerSom(int intensidade)`)**:
 *    - Método sobrecarregado para permitir rugidos múltiplos.
 *
 * O código cria um parque selvagem, adiciona setores, animais, funcionários e veterinários, e realiza várias operações, demonstrando flexibilidade e boas práticas de design.
 */
 ...
 // ✅ Classe abstrata: Define comportamento base para animais selvagens
