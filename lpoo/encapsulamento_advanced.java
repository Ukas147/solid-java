/**
 * Encapsulamento aplicado em uma Conta Bancária
 *
 * O encapsulamento é utilizado para proteger os atributos da classe `ContaBancaria`,
 * garantindo que sejam acessados e modificados apenas por meio de métodos específicos.
 *
 * Benefícios do Encapsulamento no contexto bancário:
 * - Proteção dos dados sensíveis (saldo e titular da conta).
 * - Controle sobre as operações (depósitos, saques e transferências) evitando manipulações indevidas.
 * - Registro automático das transações para manter um histórico confiável.
 * - Facilidade na manutenção e expansão do código sem comprometer a segurança da aplicação.
 *
 * A classe `ContaBancaria` implementa:
 * - Atributos privados (`titular`, `saldo` e `transacoes`) para manter a segurança dos dados.
 * - Métodos públicos para manipular saldo de forma controlada (`depositar`, `sacar`, `transferir`).
 * - Validações para evitar erros como saques maiores que o saldo ou valores inválidos.
 * - Um histórico de transações para auditoria e transparência.
 */
